...
class LokFarmer:
    def __init__(self, token, captcha_solver_config):
        ...
        # Track joined rally IDs to avoid duplicate joins
        self.joined_rally_ids = set()
    ...
    def _on_field_objects_gather(self, each_obj):
        ...
        # Tăng biến đếm khi khai thác thành công
        self.mines_gathered += 1
        # Ghi log tổng số mỏ đã khai thác
        logger.info(f"Mỏ khai thác thành công: {each_obj.get('code')} tại {to_loc}. Tổng số mỏ đã khai thác: {self.mines_gathered}")
        return True
    ...
    def sock_thread(self, join_rally_code_list=(OBJECT_CODE_DEATHKAR,)):
        ...
        def on_alliance_rally_new(data):
            ...
            logger.info(f'data_rally_new: {battles}')
            
            # Process each battle and join rallies that haven't been joined yet
            if battles:
                for battle in battles:
                    rally_id = battle.get('_id')
                    toLoc = battle.get('toLoc')
                    if rally_id and rally_id not in self.joined_rally_ids:
                        try:
                            # Prepare march troops for this rally
                            march_troops = self._prepare_march_troops(battle, MARCH_TYPE_RALLY)
                            if march_troops:  # Only join if we have troops available
                                # Call API to join rally
                                result = self.api.field_rally_join({
                                    'rallyMoId': rally_id,
                                    'marchTroops': march_troops,
                                    'toLoc': toLoc,
                                    'marchType': MARCH_TYPE_MONSTER
                                })
                                
                                if result.get('result'):
                                    # Mark this rally as joined
                                    self.joined_rally_ids.add(rally_id)
                                    logger.info(f'Successfully joined rally {rally_id} for {code}')
                                else:
                                    logger.warning(f'Failed to join rally {rally_id}: {result}')
                            else:
                                logger.info(f'No troops available for rally {rally_id}')
                        except Exception as e:
                            logger.error(f'Error joining rally {rally_id}: {e}')
    ...
    def socf_thread(self, radius, targets, share_to=None):
        ...
        def on_field_enter(data):
            ...
            logger.debug(f'field_enter_v3: {data_decoded}')
    ...
    def academy_farmer_thread(self, to_max_level=False, speedup=False):
        ...
        for category_name, each_category in RESEARCH_CODE_MAP.items():
            # Skip advanced research category
            if category_name == 'advanced':
                continue
    ...
    def skill_use(self, id_skill=(10001,)):
        """
        Use skills from the provided skill IDs.
        If a skill is in cooldown, it will be retried after the cooldown period.
        
        Args:
            id_skill (tuple): Tuple of skill IDs to use. Defaults to (10001,)
        """
        if not id_skill:
            return

        try:
            for skill_id in id_skill:
                try:
                    self.api.use_skill(skill_id)
                    logger.debug(f'Successfully used skill {skill_id}')
                except OtherException as error:
                    if str(error) == 'yet_in_cooltime':
                        logger.info(f'Skill {skill_id} in cooldown, scheduling retry in 1h')
                        threading.Timer(3600, self.skill_use, [(skill_id,)]).start()
                    else:
                        logger.error(f'Error using skill {skill_id}: {error}')
                        raise
        except Exception as e:
            logger.error(f'Unexpected error in skill_use: {e}')
            raise
